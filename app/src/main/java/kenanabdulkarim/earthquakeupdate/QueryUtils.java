...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            // magnitude, place, time and ... are stored inside the features array
            // So, we want the values associated with the features key
            // It should map to a JSON array that contains a list of earthquakes
            // So, call call getJSONArray method on baseJsonResponse object
            // passing in the features key as a string
            JSONArray earthquakeArray = baseJsonResponse.getJSONArray("features");
            // store result now in earthquake array variables

            for (int i = 0; i < earthquakeArray.length(); i++) {
                JSONObject currentEarthquake = earthquakeArray.getJSONObject(i);
                JSONObject properties = currentEarthquake.getJSONObject("properties");
                // we are now able to access the individual values of the properties JSONObject
                // by referring to the key mag for magnitude, place location and time
